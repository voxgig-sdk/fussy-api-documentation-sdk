...
// FussyApiDocumentation SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class FussyApiDocumentationTestFeature extends FussyApiDocumentationBaseFeature
{
    ...
    public function init(FussyApiDocumentationContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (FussyApiDocumentationContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = FussyApiDocumentationParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (FussyApiDocumentationContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(FussyApiDocumentationContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = FussyApiDocumentationParam::call($ctx, $k);
