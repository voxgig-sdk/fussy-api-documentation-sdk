...
//    only - a typed FussyApiDocumentationContext there would make every ctx
...
final class FussyApiDocumentationOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new FussyApiDocumentationContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new FussyApiDocumentationSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new FussyApiDocumentationResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new FussyApiDocumentationError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new FussyApiDocumentationResponse($clean['response']);
    ...
    public static function err_from_map($m): ?FussyApiDocumentationError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new FussyApiDocumentationError(is_string($code) ? $code : '', $msg);
    }
