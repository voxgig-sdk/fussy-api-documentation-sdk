...
class FussyApiDocumentationSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = FussyApiDocumentationHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = FussyApiDocumentationHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!FussyApiDocumentationFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, FussyApiDocumentationFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return FussyApiDocumentationUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = FussyApiDocumentationHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = FussyApiDocumentationHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = FussyApiDocumentationHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new FussyApiDocumentationError($op . "_allow",
                "FussyApiDocumentationSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = FussyApiDocumentationHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new FussyApiDocumentationError("graphql_error",
                "FussyApiDocumentationSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    // Canonical facade: $client->GraphQl_()->list() / ->load(["id" => ...]).
    // Renamed from GraphQl: that name is already taken by an SDK class
    // member, and a duplicate declaration is a fatal PHP parse error.
    public function GraphQl_($data = null)
    {
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new FussyApiDocumentationSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
