...
// FussyApiDocumentation SDK retry feature
...
class FussyApiDocumentationRetryFeature extends FussyApiDocumentationBaseFeature
{
    ...
    public function init(FussyApiDocumentationContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (FussyApiDocumentationContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(FussyApiDocumentationContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(FussyApiDocumentationContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
