...
// FussyApiDocumentation SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = FussyApiDocumentationOmni::makeStructRunner(
    ...
    {
        return FussyApiDocumentationOmni::objmodel(FussyApiDocumentationOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return FussyApiDocumentationOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return FussyApiDocumentationOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(FussyApiDocumentationOmni::structfix($now, Struct::undef()))
                        !== json_encode(FussyApiDocumentationOmni::structfix($val, Struct::undef()))) {
