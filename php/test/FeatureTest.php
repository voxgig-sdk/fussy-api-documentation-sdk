...
// FussyApiDocumentation SDK feature test
//
...
class FtCtrl extends FussyApiDocumentationControl
{
...
class FtClient
{
    ...
    private FussyApiDocumentationUtility $utility;

    public function __construct(FussyApiDocumentationUtility $utility, array $options)
    {
    ...
    public function get_utility(): FussyApiDocumentationUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (FussyApiDocumentationContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public FussyApiDocumentationContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (FussyApiDocumentationContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = FussyApiDocumentationConfig::make_feature($name);
    ...
    private static function buildUrl(FussyApiDocumentationSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof FussyApiDocumentationError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new FussyApiDocumentationResponse($fetched);
        }
    ...
    private function fail(FussyApiDocumentationContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (FussyApiDocumentationContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
