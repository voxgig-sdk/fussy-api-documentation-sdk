...
local GraphQlEntity = {}
GraphQlEntity.__index = GraphQlEntity


function GraphQlEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GraphQlEntity)
  self._name = "graph_ql"
  self._client = client
...
function GraphQlEntity:get_name()
  return self._name
end


function GraphQlEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GraphQlEntity.new(self._client, opts)
end
...
function GraphQlEntity:mark_deleted()
  self._deleted = true
end


function GraphQlEntity:deleted()
  return true == self._deleted
end


function GraphQlEntity:data_set(args)
  if args ~= nil then
...
function GraphQlEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GraphQlEntity:match_set(args)
  if args ~= nil then
...
function GraphQlEntity:match_get()
...
function GraphQlEntity:stream(action, args, callopts)
...
function GraphQlEntity:_run_op(ctx, post_done)
  local utility = self._utility
