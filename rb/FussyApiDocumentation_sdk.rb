...
require_relative 'FussyApiDocumentation_types'
...
class FussyApiDocumentationSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = FussyApiDocumentationHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = FussyApiDocumentationHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, FussyApiDocumentationFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    FussyApiDocumentationUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = FussyApiDocumentationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = FussyApiDocumentationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = FussyApiDocumentationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => FussyApiDocumentationError.new(
        "#{op}_allow",
  ...
  def raw_request(fetchargs = {})
    ...
    rescue FussyApiDocumentationError => err
    ...
    if fetched.is_a?(Hash)
      status = FussyApiDocumentationHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = FussyApiDocumentationError.new(
        "graphql_error", "FussyApiDocumentationSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = FussyApiDocumentationSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
