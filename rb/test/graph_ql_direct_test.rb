...
class GraphQlDirectTest < Minitest::Test
  def test_direct_list_graph_ql
    setup = graph_ql_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-graph_ql", setup[:live] ? "live" : "unit")
    if _should_skip
...
def graph_ql_direct_setup(mockres)
