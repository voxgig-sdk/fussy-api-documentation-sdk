...
class GraphQlEntity
  ...
  def make
    opts = @_entopts.dup
    GraphQlEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = FussyApiDocumentationHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = FussyApiDocumentationHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #
  # @param reqmatch [GraphQlListMatch, Hash, nil] match filter (any subset of
  #   GraphQl fields); defaults to nil, treated as an empty match that lists all.
    ...
    _run_op(ctx) do
      if ctx.result
        if ctx.result.resdata
          @_data = FussyApiDocumentationHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
