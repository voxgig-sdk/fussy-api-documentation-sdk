...
module FussyApiDocumentationUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = FussyApiDocumentationResponse.new({})
    ...
    if fetch_err
      ...
      response = FussyApiDocumentationResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = FussyApiDocumentationResponse.new(fetched)
    else
