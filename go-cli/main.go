// Command FussyApiDocumentation-cli is a CLI + REPL for the FussyApiDocumentation SDK,
...
const prompt = "fussy-api-documentation"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "graph_ql"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      FUSSY_API_DOCUMENTATION_APIKEY (api key), FUSSY_API_DOCUMENTATION_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: FUSSY_API_DOCUMENTATION_APIKEY carries the API key and
	// FUSSY_API_DOCUMENTATION_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("FUSSY_API_DOCUMENTATION_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("FUSSY_API_DOCUMENTATION_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewFussyApiDocumentationSDK(opts)
