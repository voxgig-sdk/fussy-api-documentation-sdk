...
boru-driven command-line client **and** interactive REPL for the FussyApiDocumentation
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/fussy-api-documentation-cli)
make build

# 2. See usage (words, entities, env vars)
./fussy-api-documentation-cli --help
...
FUSSY_API_DOCUMENTATION_BASE=https://api.example.com ./fussy-api-documentation-cli list graph_ql

# 6. No arguments -> interactive REPL
./fussy-api-documentation-cli
fussy-api-documentation> list graph_ql
fussy-api-documentation> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/fussy-api-documentation-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/fussy-api-documentation-cli list graph_ql
   ```
...
Run with no arguments to open a REPL (prompt `fussy-api-documentation>`). Each line is
evaluated as its own boru expression:

```text
$ ./fussy-api-documentation-cli
fussy-api-documentation> list graph_ql
fussy-api-documentation> /help
fussy-api-documentation> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `graph_ql`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/fussy-api-documentation-cli`. |
...
(`./fussy-api-documentation-cli <expr>`) and interactively (the REPL), and expressions compose the
