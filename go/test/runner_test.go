...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("FUSSYAPIDOCUMENTATION_TEST_LIVE") == "TRUE" ||
		os.Getenv("FUSSYAPIDOCUMENTATION_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("FUSSYAPIDOCUMENTATION_TEST_EXPLAIN"); explain != "" {
