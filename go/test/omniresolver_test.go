...
import (
	...
	omni "github.com/voxgig-sdk/fussy-api-documentation-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.FussyApiDocumentationError); is {
				out := map[string]any{
					"name":    "FussyApiDocumentationError",
...
func omniCtx(arg any, client *sdk.FussyApiDocumentationSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.FussyApiDocumentationSDK); is {
			client = live
