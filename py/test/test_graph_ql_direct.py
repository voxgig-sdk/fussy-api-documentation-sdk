...
class TestGraphQlDirect:

    def test_should_direct_list_graph_ql(self):
        setup = _graph_ql_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-graph_ql", "live" if setup["live"] else "unit")
        if _skip:
...
def _graph_ql_direct_setup(mockres):
