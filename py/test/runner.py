# FussyApiDocumentation SDK test runner
...
class FussyApiDocumentationTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            FussyApiDocumentationTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = FussyApiDocumentationTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = FussyApiDocumentationTestRunner.getenv("FUSSY_API_DOCUMENTATION_TEST_LIVE")
        override = FussyApiDocumentationTestRunner.getenv("FUSSY_API_DOCUMENTATION_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = FussyApiDocumentationTestRunner.getenv(key)
        ...
        explain = FussyApiDocumentationTestRunner.getenv("FUSSY_API_DOCUMENTATION_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["FUSSY_API_DOCUMENTATION_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if FussyApiDocumentationTestRunner._test_control is not None:
            return FussyApiDocumentationTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                FussyApiDocumentationTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            FussyApiDocumentationTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = FussyApiDocumentationTestRunner.load_test_control()
...
def load_env_local():
    FussyApiDocumentationTestRunner.load_env_local()


def env_override(m):
    return FussyApiDocumentationTestRunner.env_override(m)


def entity_data(v):
    return FussyApiDocumentationTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return FussyApiDocumentationTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return FussyApiDocumentationTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return FussyApiDocumentationTestRunner.load_test_control()


def live_client_options():
    return FussyApiDocumentationTestRunner.live_client_options()


def live_delay_ms():
    return FussyApiDocumentationTestRunner.live_delay_ms()
