# fussy-api-documentation-mcp

[MCP](https://modelcontextprotocol.io) server exposing the FussyApiDocumentation SDK as
two agent tools — `fussy-api-documentation_list` and `fussy-api-documentation_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/fussy-api-documentation-mcp)
make build

# 2. Provide credentials via the environment
export FUSSY_API_DOCUMENTATION_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user fussy-api-documentation \
  -- /absolute/path/to/fussy-api-documentation-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./fussy-api-documentation-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// fussy-api-documentation_list: first page of records
{ "entity": "graph_ql" }
{ "entity": "graph_ql", "query": { } }

// fussy-api-documentation_load: one record by id
{ "entity": "graph_ql", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/fussy-api-documentation-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user fussy-api-documentation \
     -- "$PWD"/dist/*/fussy-api-documentation-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `fussy-api-documentation_list` and `fussy-api-documentation_load` tools now appear
   in new sessions. Ask the agent to *"list graph_ql using fussy-api-documentation"*
   and it calls `fussy-api-documentation_list` with `{"entity":"graph_ql"}`.
...
```sh
export FUSSY_API_DOCUMENTATION_APIKEY=sk_live_xxx            # API key
export FUSSY_API_DOCUMENTATION_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `fussy-api-documentation_list` tool
...
### Call the `fussy-api-documentation_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `fussy-api-documentation_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `fussy-api-documentation_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `FUSSY_API_DOCUMENTATION_APIKEY` | API key sent with every request. |
| `FUSSY_API_DOCUMENTATION_BASE` | Optional override of the API base URL. |
...
```sh
./fussy-api-documentation-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"fussy-api-documentation_load","arguments":{"entity":"graph_ql","query":{"id":1}}}}'
```
