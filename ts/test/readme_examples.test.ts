...
//     (\`new FussyApiDocumentationSDK(...)\` / \`FussyApiDocumentationSDK.test(...)\` become
//     \`FussyApiDocumentationSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'FussyApiDocumentationSDK'
...
const TEST_SEED = {"entity":{"graph_ql":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new FussyApiDocumentationSDK(...)\` and any
// \`FussyApiDocumentationSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+FussyApiDocumentationSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/FussyApiDocumentationSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
