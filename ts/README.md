# FussyApiDocumentation TypeScript SDK



The TypeScript SDK for the FussyApiDocumentation API — a type-safe, entity-oriented client with full async/await support.

The API is exposed as capitalised, semantic **Entities** — e.g.
`client.GraphQl()` — each with a small set of operations (`list`, `create`)
...
```ts
const client = new FussyApiDocumentationSDK({ apikey: '...' })
...
```ts
new FussyApiDocumentationSDK(options?: {
  apikey?: string
  base?: string
  prefix?: string
  suffix?: string
...
| `tester(testopts?, sdkopts?)` | `FussyApiDocumentationSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `FussyApiDocumentationSDK.test(testopts?, sdkopts?)` | `FussyApiDocumentationSDK` | Create a test-mode client. |
...
| `client` | `client(): FussyApiDocumentationSDK` | Return the parent SDK client. |
...
├── src/
│   ├── FussyApiDocumentationSDK.ts        # Main SDK class
